...
**Status:** implemented on `feat/25-source-derivative`. Tasks 1–13 done, 14 (documentation) in progress, 15 (final verification) pending. Deviations are recorded in the spec's third revision note and, at length, in the commit that made each one — read `git log origin/main..HEAD` before assuming a task's text still describes the code.
...
- **Pin `plone.namedfile < 8` for this work — and know that `uv run` undoes it.** There is no lockfile in the repository and the dependency is unbounded, so a fresh environment resolves 8.1.1 while the target deployment runs 7.x. `uv pip install "plone.namedfile<8"` sets the pin; the very next plain `uv run pytest` re-syncs against `pyproject.toml` and silently restores 8.1.1. Verified, not theoretical. Run the suite as `UV_NO_SYNC=1 uv run pytest` (or `.venv/bin/python -m pytest`) and check the version whenever something behaves unexpectedly: `.venv/bin/python -c "import importlib.metadata as m; print(m.version('plone.namedfile'))"`. The version flips `_HAS_SCALE_URL` (`scaling.py:208`) and with it whether `ThumborImageScale.__init__` or `._scale_url` is the live path. `_build_thumbor_url` is shared, so no implementation fork is needed — but a test suite written under one resolution silently never exercises the other. Assert the §4 behaviours through both entry points regardless of which is live.
...
"Neither attribute present" — **or** a recorded reason that is non-terminal, **or** a recorded `max_edge` that differs from the configured cap — is what makes an image a backfill candidate. Recording the cap is what turns tuning it from a migration into a setting change: `plone.pgthumbor` is generic, every deployment will pick its own value at least once, and a re-run that silently does nothing without `force` is a trap. Terminal reasons (`"generated"`, `"not_needed"`, `"skipped_type"`) mean a re-run would change nothing. (As built, `"skipped_type"` is recorded only for SVG, decided on `contentType` before any bytes are read; an animated GIF falls out of `build_derivative_bytes` as `None` and is recorded `"not_needed"`. Both are terminal, so nothing depends on telling them apart — but the plan's original wording, which put animated GIFs under `"skipped_type"`, does not describe the code.) Non-terminal reasons (`"retry"` from a semaphore timeout, `"error"` from a failed decode) are transient and must be re-selected by an ordinary backfill run, without `force`. Getting this wrong lets one contended upload exclude its image permanently while the terminal verification still reports success. `_pgthumbor_source_info["source_ids"]` is compared against `get_blob_ids(original)` at URL-build time so an in-place `image.data = ...` mutation cannot leave a stale derivative in service.
...
- [ ] Tests first: small clean JPEG → `None`; oversized JPEG → capped JPEG at the expected decoded size; **CMYK under cap → RGB with dimensions unchanged**; oversized RGBA → PNG; palette+transparency → PNG; animated GIF → `None`; SVG → `None`; corrupt bytes → `None` plus a WARNING; file object accepted; `max_edge=0` → `None`; EXIF dropped; JPEG subsampling is 0.
- [ ] **Truncated bytes are not the same case as corrupt bytes.** An earlier revision of this plan expected both to return `None`. `plone/scale/scale.py:53` sets `PIL.ImageFile.LOAD_TRUNCATED_IMAGES = True` at import, process-wide, so a truncated source decodes with grey padding and *does* produce a derivative. That is the decision (spec §3): match what Plone already renders for those bytes rather than judge them more harshly. Pin it with a test, so nobody later "fixes" it into silence, and pin the flag explicitly in any test that depends on it — whether `scaling` has been imported yet is a test-ordering accident.
...
- [ ] **Do not** skip the derivative when the encoded result is not smaller than the source, despite what an earlier revision of this plan said. The reasoning does not survive contact with the primary goal: byte size and pixel count are independent, so a highly compressed 11811 px original can be *smaller* than its 4000 px derivative while still being the 104 MP image Thumbor refuses. Skipping there would reintroduce the exact HTTP 400 this design removes. Every trigger has a reason the derivative must win — size for the guarantee, colour for correctness — so the rule has no safe domain. Log a warning when the derivative comes out larger, so the dry run and operators can see the storage cost, and keep it.
